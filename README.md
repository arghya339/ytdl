...
<p align="center">
A feature-rich command-line Video/Audio Downloader for macOS/Android, based on yt-dlp.
<br>
<br>
...
- Download SD Video Format in MP4 and HD /FULL HD /FHD /HQ /UHD Video in MKV for Smaller file Sized
- Preferred Video Codec VP09 for UHD/ HQ video and AV01 Codec for FULL HD/ HD /SD video
...
- Download images as PNG/JPG/WEBP/GIF from various websites (cloudflare unprotect site only), tested on YT and YT Music.
- Built in Updating feature
- User Friendly and FOSS
- Smaller size script (lessthen 40 KB), allowing you to save space on your device

## Prerequisites
- macOS / Android device with working internet connection.

## Usage
- Open macOS Terminal / [Termux](https://github.com/termux/termux-app/releases) and run the script with the following command:
```sh
curl -L --progress-bar -o "$HOME/.ytdl.sh" "https://raw.githubusercontent.com/arghya339/ytdl/refs/heads/main/bash/ytdl.sh" && bash ~/.ytdl.sh
```
...
```
> [!TIP]
> Please uncheck `Resotre text when reopening windows` option in macOS Terminal > Settings > Profiles > Window
